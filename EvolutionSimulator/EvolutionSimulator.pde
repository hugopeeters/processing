/*
** Evolution Simulator.
** This is a Tweak of the Actually Improved Evolution Simulator by Cary Huang
**
** I am making the code more readable to me and including comments when I
** figure out what sections of code are doing.
**
** I am also altering text and other GUI features to make it more user-friendly.
**
** In all, Huang's program is an awesome introduction to genetic algorithms.
**
** "By the old gods and the new, COMMENT YOUR CODE." ~ Anon.
*/
 
final float windowSizeMultiplier = 0.9;
final int SEED = 0;
...
void drawScreenImage(int stage) {
  screenImage.smooth();
  screenImage.beginDraw();screenImage.pushMatrix();
 ...
 // screenImage.textFont(font, 24);
  ...
  if (stage == 0) {
    ...
    screenImage.text("Time to test them.", windowWidth/2-130, 700);
    screenImage.text("Test generation", windowWidth-150, 700);
  }
  screenImage.endDraw();
  screenImage.popMatrix();
}
...
/* Begin program. Display title and start button.
*/
void draw()
{
  scale(windowSizeMultiplier); // Big window or small window.
  if (menu == 0)
  {
    background(255);
    fill(100, 200, 100);
    ...
    fill(0);
    text("The Evolution Machine", windowWidth/2, 200);
    text("START", windowWidth/2, 430);
  }
  else if (menu == 1) // Menu1 is the beginning menu, where the creatures are initially created
  {
    noStroke();
    fill(0);
    background(255, 200, 130); // Background colour
   // textFont(font, 32);
    textAlign(LEFT);
   // textFont(font, 96);
    ...
    if (gen == -1)
    {
      ...
      rect(20, 250, 200, 100);
      fill(0);
      text("We will begin by creating 1000 random creatures.", 20, 160);
      text("They will have very simple forms.", 20, 200);
      text("CREATE", 25, 312);
    }
    else
    {
  ...
  } else if (menu == 2)
  {
    ...
    textAlign(CENTER);
 //   textFont(font, 24);
    text("The initial 1000 randomly generated creatures.", windowWidth/2-200, 690); // Initial creatures
    text("Test generation", windowWidth-250, 690);
  }else if (menu == 4)
  {
  ...
  if (menu == 8)  // cool sorting animation
  {
    background(220, 253, 102);
    pushMatrix();
    scale(10.0/scaleToFixBug);
